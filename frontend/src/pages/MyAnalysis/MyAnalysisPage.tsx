...
export function MyAnalysisPage() {
  ...
  const states = useMemo(() => [upbit, kospi, kosdaq, nasdaq, nyse], [kosdaq, kospi, nasdaq, nyse, upbit])
  ...
  const cryptoCandidates = useMemo(() => buildCryptoWatchCandidates({ instruments: upbit.catalog?.instruments ?? [], newsResult, language, limit: 10 }), [language, newsResult, upbit.catalog?.instruments])
  ...
  const analysis = selected ? buildMyInstrumentAnalysis({ instrument: selected, intent, displayMode, userNote: note, averagePrice: parsedAverage !== null && Number.isFinite(parsedAverage) ? parsedAverage : null, catalogSource: sourceById.get(selected.id) ?? null, candidate, newsResult, language }) : null
  ...
  return <main className={styles.page}>
    <header className={styles.header}><div><span>{t.eyebrow}</span><h1>{t.title}</h1><p>{t.description}</p></div><div className={styles.mode} aria-label={t.analyze}><span>{displayMode === 'simple' ? t.simple : t.expert}</span>{displayMode === 'simple' && <button type="button" onClick={() => setDisplayMode('expert')}>{t.switchExpert}</button>}</div></header>
    <section className={styles.searchPanel} aria-label={t.search}>
      ...
      {normalized && <div className={styles.results} role="listbox" aria-label={t.search}>{results.map((instrument) => <button type="button" role="option" aria-selected={selectedId === instrument.id} key={instrument.id} onClick={() => choose(instrument)}><strong>{instrument.displaySymbol ?? instrument.symbol}</strong><span>{instrument.koreanName ?? instrument.name} · {instrument.englishName ?? instrument.name}</span><em>{instrument.marketType?.toUpperCase() ?? instrument.marketId}</em></button>)}{!loading && results.length === 0 && <p>{t.noResults}</p>}</div>}
    ...
    {selected && analysis && <>
      ...
      <section className={styles.analysis} data-mode={displayMode}><div className={styles.analysisHeading}><div><span>{displayMode === 'simple' ? t.simple : t.expert}</span><h2>{t.analyze}</h2><p>{analysis.summary}</p></div><Link to="/market" onClick={openMarket}>{t.market} →</Link></div><div className={styles.sections}>{(displayMode === 'simple' ? analysis.simpleModeSections : analysis.expertModeSections).map((section) => <article key={section.title}><h3>{section.title}</h3>{section.items.length ? <ul>{section.items.map((entry) => <li key={entry}>{entry}</li>)}</ul> : <p>{t.noItems}</p>}</article>)}</div><footer>{analysis.disclaimer}</footer></section>
    </>}
  </main>
}
