...
const copy = {
  en: {
    ...
    search: 'Search instrument', searchPlaceholder: 'Search symbol, Korean name, or English name', empty: 'Search and select an instrument to begin.', noResults: 'No matching instrument in the loaded catalogs.', loading: 'Loading market catalogs…', unavailable: 'The requested instrument is not available in the loaded catalogs.', catalogLimit: 'Market catalog limitation', catalogError: 'Some market catalogs could not be loaded. Search results may be incomplete.',
    intent: 'My intent', intents: { watching: 'Watching', holding: 'Holding', longTerm: 'Long-term review', swing: 'Swing review', shortTerm: 'Short-term review' }, note: 'My note (optional)', notePlaceholder: 'Personal context only — not treated as market evidence', average: 'Average price (optional)', analyze: 'Analysis context', simple: 'Simple view', expert: 'Expert view', switchExpert: 'Switch to Expert view', switchSimple: 'Switch to Simple view', input: 'Input summary', yourInputs: 'Your inputs', market: 'Open in Market', quality: 'Evidence quality', noItems: 'Nothing available yet.', selected: 'Selected instrument', source: 'Source', evidence: 'Evidence board', missing: 'Missing evidence', checklist: 'Review checklist', levels: { available: 'Available', context: 'Context', demo: 'Demo', missing: 'Missing' },
  },
  ko: {
    eyebrow: '개인 리서치 작업공간', title: '내 종목 분석', description: '종목 하나를 선택하고 확인된 내용, 부족한 근거, 다음 검토 항목을 정리합니다.',
    search: '종목 검색', searchPlaceholder: '심볼·한국어 이름·영어 이름 검색', empty: '분석을 시작하려면 종목을 검색하고 선택하세요.', noResults: '불러온 카탈로그에서 일치하는 종목이 없습니다.', loading: '시장 카탈로그를 불러오는 중…', unavailable: '요청한 종목을 불러온 카탈로그에서 찾을 수 없습니다.', catalogLimit: '시장 카탈로그 제한', catalogError: '일부 시장 카탈로그를 불러오지 못했습니다. 검색 결과가 일부 누락될 수 있습니다.',
    intent: '나의 목적', intents: { watching: '관찰 중', holding: '보유 중', longTerm: '장기 검토', swing: '스윙 검토', shortTerm: '단기 검토' }, note: '내 메모 (선택)', notePlaceholder: '개인 참고 맥락이며 시장 근거로 취급하지 않습니다', average: '평균 가격 (선택)', analyze: '분석 맥락', simple: '간편 보기', expert: '전문가 보기', switchExpert: '전문가 보기로 전환', switchSimple: '간편 보기로 전환', input: '입력 요약', yourInputs: '내 입력값', market: '마켓에서 열기', quality: '근거 품질', noItems: '아직 확인된 항목이 없습니다.', selected: '선택 종목', source: '출처', evidence: '근거 보드', missing: '부족한 근거', checklist: '검토 체크리스트', levels: { available: '사용 가능', context: '참고', demo: '데모', missing: '부족' },
  },
} as const
    ...
    {selected && analysis && <>
      ...
      <section className={styles.analysis} data-mode={displayMode}><div className={styles.analysisHeading}><div><span>{displayMode === 'simple' ? t.simple : t.expert}</span><h2>{t.analyze}</h2><p>{analysis.summary}</p></div><Link to="/market" onClick={openMarket}>{t.market} →</Link></div>
        {displayMode === 'simple' ? <div className={styles.sections}>{analysis.simpleModeSections.map((section) => <article key={section.title}><h3>{section.title}</h3>{section.items.length ? <ul>{section.items.map((entry) => <li key={entry}>{entry}</li>)}</ul> : <p>{t.noItems}</p>}</article>)}</div>
          : <div className={styles.expertGrid}><article><h3>{expertTitles.evidence ?? t.evidence}</h3><div className={styles.evidenceList}>{analysis.evidence.map((entry) => <div key={entry.id} className={styles.evidenceItem}><header><strong>{entry.label}</strong><span data-level={entry.level}>{t.levels[entry.level]}</span></header><p>{entry.detail}</p><small>{t.source}: {entry.source}</small></div>)}</div></article><article><h3>{expertTitles.missing ?? t.missing}</h3><div className={styles.evidenceList}>{analysis.missingEvidence.map((entry) => <div key={entry.id} className={styles.evidenceItem}><header><strong>{entry.label}</strong><span data-level={entry.level}>{t.levels[entry.level]}</span></header><p>{entry.detail}</p><small>{t.source}: {entry.source}</small></div>)}</div></article><article><h3>{expertTitles.checklist ?? t.checklist}</h3><ul>{analysis.reviewChecklist.map((entry) => <li key={entry}>{entry}</li>)}</ul></article></div>}
