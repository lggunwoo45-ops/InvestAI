  ...
  return <main className={styles.page}>
    ...
    {selected && analysis && <>
      ...
      <section className={styles.instrumentHeader} aria-label={t.selected}>
        ...
        <div className={styles.quote}><small>{t.availableData}</small><strong>{formatMarketPrice(selected)}</strong><span data-direction={selected.change24hPercent >= 0 ? 'positive' : 'negative'}>{formatMarketChange(selected.change24hPercent)}</span></div>
      ...
      <section className={styles.analysis} data-mode={displayMode}>
        <div className={styles.analysisHeading}><div><span>{displayMode === 'simple' ? t.simple : t.expert}</span><h2>{t.analyze}</h2><p>{analysis.summary}</p></div><Link to="/market" onClick={openMarket}>{t.market} →</Link></div>
        {displayMode === 'simple' ? <div className={styles.sections}>
          {analysis.simpleModeSections.map((section) => <article key={section.id}><h3>{section.title}</h3>{section.items.length ? <ul>{section.items.slice(0, 3).map((entry) => <li key={entry}>{entry}</li>)}</ul> : <p>{t.noItems}</p>}</article>)}
          <article><h3>{t.nextChecks}</h3><ul>{analysis.reviewChecklist.slice(0, 3).map((entry) => <li key={entry}>{entry}</li>)}</ul></article>
        </div> : <div className={styles.expertGrid}>
          <article><span className={styles.sectionLabel}>{t.availableData}</span><h3>{expertTitles.evidence ?? t.evidence}</h3><div className={styles.evidenceList}>{analysis.evidence.map((entry) => <div key={entry.id} className={styles.evidenceItem}><header><strong>{entry.label}</strong><span data-level={entry.level}>{t.levels[entry.level]}</span></header><small className={styles.evidenceType}>{t.type}: {entry.type}</small><p>{entry.detail}</p><small>{t.source}: {entry.source}</small></div>)}</div></article>
