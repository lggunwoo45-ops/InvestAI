...
describe('MyAnalysisPage', () => {
  ...
  it('starts empty, selects an instrument, and switches the global detail level', async () => {
    ...
    expect(screen.getAllByRole('heading', { name: 'Next checks' }).length).toBeGreaterThan(1)
    ...
    expect(actionStatus.textContent).toContain('This is a workflow preview using mock/demo data.')
    expect(actionStatus.compareDocumentPosition(screen.getByRole('heading', { name: 'What is visible now' })) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    ...
    expect(screen.getByRole('region', { name: 'Action brief' }).textContent).toContain('See rule basis below.')
  ...
  it('supports direct instrument query selection and bilingual navigation', async () => {
    ...
    const koreanBrief = screen.getByRole('region', { name: '액션 브리핑' })
    expect(koreanBrief.textContent).toContain('데이터 신뢰 상태')
    expect(koreanBrief.textContent).toContain('아래 규칙 근거를 확인하세요.')
