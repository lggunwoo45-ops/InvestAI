...
describe('MyAnalysisPage', () => {
  ...
  it('starts empty, selects an instrument, and switches the global detail level', async () => {
    ...
    expect(screen.getByText('Stock price zones will be available after reliable stock data is connected.')).toBeTruthy()
  ...
  it('supports direct instrument query selection and bilingual navigation', async () => {
    ...
    expect(readiness.textContent).toContain('About 1.5% below current price')
    expect(readiness.textContent).not.toMatch(/KRW|USD|USDT/)
    ...
    expect(screen.getByRole('region', { name: '액션 준비도' }).textContent).toContain('거래 지시가 아닙니다')
    expect(screen.getByText('현재가보다 약 1.5% 아래')).toBeTruthy()
    ...
    expect(document.body.textContent).not.toMatch(/매수가|손절가|익절가|목표가|매수 추천/)
  })
