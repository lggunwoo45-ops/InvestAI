...
describe('MyAnalysisPage', () => {
  ...
  it('starts empty, selects an instrument, and switches the global detail level', async () => {
    ...
    expect(within(actionStatus).getByText('Signal clarity: Low')).toBeTruthy()
  ...
  it('supports direct instrument query selection and bilingual navigation', async () => {
    ...
    expect(readiness.textContent).toContain('Signal clarity describes how clear the current rule-based status is, not confidence or expected return.')
    ...
    expect(koreanReadiness.textContent).toContain('판단 명확도는 현재 규칙 기반 상태의 명확도를 뜻하며, 확신도나 기대수익이 아닙니다.')
