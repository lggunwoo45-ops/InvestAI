...
describe('Market Copilot application shell', () => {
  ...
  it('provides persisted multi-watchlist dashboard controls', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Top radar signals' })).toBeTruthy()
    expect(screen.queryByRole('heading', { name: 'Today’s Market Summary' })).toBeNull()
    expect(screen.getByRole('tab', { name: /Crypto/ })).toBeTruthy()
    expect(screen.getByRole('tab', { name: /Korea/ })).toBeTruthy()
    expect(screen.getByRole('tab', { name: /US/ })).toBeTruthy()
  ...
  it('shows safe Simple Mode guidance across major pages while preserving their routes', async () => {
    ...
    render(<App />)
    fireEvent.click(screen.getByRole('button', { name: /Simple Mode/ }))
    expect(await screen.findByRole('heading', { name: 'Today’s Market Summary' })).toBeTruthy()
    expect(screen.getByText('Show detailed radar')).toBeTruthy()
    expect(screen.getByRole('heading', { name: 'Risk notes' })).toBeTruthy()
    fireEvent.click(screen.getByRole('link', { name: 'Market' }))
    expect(await screen.findByText('Search a coin or stock to review price, movement, and basic context.')).toBeTruthy()
    fireEvent.click(screen.getByRole('link', { name: 'News' }))
    expect(await screen.findByText('Start with the headline, source, and why it may matter.')).toBeTruthy()
    fireEvent.click(screen.getByRole('link', { name: 'Demo' }))
    expect(await screen.findByRole('heading', { name: 'Simple demo path' })).toBeTruthy()
    expect(screen.getByRole('link', { name: 'Open Simple Candidate View' }).getAttribute('href')).toBe('/simple')
  })
