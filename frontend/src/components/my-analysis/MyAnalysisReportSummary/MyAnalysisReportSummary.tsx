...
function buildMyAnalysisPlainTextSummary(analysis: MyAnalysisResult, symbol: string, language: Language, reviewMode: MyAnalysisReviewMode, basisPrice: number | null, currentPrice: number, quoteCurrency: string) {
  ...
  if (reviewMode === 'position' && basisPrice !== null && Number.isFinite(basisPrice) && basisPrice > 0) {
    ...
    return [t.plainTitle, symbol, `${t.position}: ${t.positions[position.state]}`, `${t.basis}: ${formatValue(basisPrice, language)} ${quoteCurrency}`, `${t.currentPrice}: ${Number.isFinite(currentPrice) ? `${formatValue(currentPrice, language)} ${quoteCurrency}` : '—'}`, `${t.basisChange}: ${position.percentChange === null ? '—' : `${position.percentChange >= 0 ? '+' : ''}${formatValue(position.percentChange, language)}%`}`, `${t.next}: ${next}`, t.safety].join('\n')
  }
...
export function MyAnalysisReportSummary({ analysis, language, mode, symbol, name, reviewMode = 'interest', basisPrice = null, currentPrice = Number.NaN, quoteCurrency = '' }: MyAnalysisReportSummaryProps) {
  const t = copy[language]
  const [copyState, setCopyState] = useState<'idle' | 'copied' | 'unavailable'>('idle')
  const plainText = useMemo(() => buildMyAnalysisPlainTextSummary(analysis, symbol, language, reviewMode, basisPrice, currentPrice, quoteCurrency), [analysis, basisPrice, currentPrice, language, quoteCurrency, reviewMode, symbol])
