...
const copy = {
  en: {
    current: 'Current action status', readiness: 'Action readiness', clarity: 'Signal clarity', clarityHelp: 'Signal clarity describes how clear the current rule-based status is, not confidence or expected return.', why: 'Why this status', check: 'Conditions to check', avoid: 'Conditions to avoid', next: 'Next checks', strengths: { low: 'Low', medium: 'Medium', high: 'High' },
  },
  ko: {
...
export function ActionReadinessCard({ plan, language, mode }: ActionReadinessCardProps) {
  ...
  return <section className={styles.card} data-mode={mode} aria-label={title}>
    <header>
      ...
      <div className={styles.statusMeta}><b data-strength={plan.strength}>{t.clarity}: {t.strengths[plan.strength]}</b><p>{t.clarityHelp}</p></div>
    </header>
