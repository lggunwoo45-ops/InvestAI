...
export function MarketRadar({ snapshot, language, onOpenInstrument, simpleMode = false }: MarketRadarProps) {
  ...
  const signalPanel = <section className={styles.signals} aria-labelledby="radar-signals-title"><h2 id="radar-signals-title">{t.top}</h2>{topSignals.length ? <ol>{topSignals.map((signal) => <li key={signal.id} data-status={signal.status}><header><span>{typeLabels[language][signal.type]} · {signal.scope}</span><em>{t.statuses[signal.status]}</em></header><strong>{signal.title}</strong><p>{signal.summary}</p><div className={styles.evidence}><span>{signal.evidenceLabel}</span><span>{t.sources[signal.source]}</span></div><div className={styles.symbols}>{signal.relatedSymbols.map((symbol) => <b key={symbol}>{symbol}</b>)}</div><footer>{signal.relatedInstrumentIds[0] ? <button type="button" onClick={() => onOpenInstrument(signal.relatedInstrumentIds[0])}>{t.open} →</button> : <span>{signal.scope === 'macro' || signal.relatedMarkets.length ? t.marketLevel : t.noInstrument}</span>}</footer></li>)}</ol> : <div className={styles.empty}>{t.statuses.incomplete}</div>}</section>
  const riskPanel = <aside className={styles.risks} aria-labelledby="radar-risks-title"><h2 id="radar-risks-title">{t.risk}</h2><ul>{snapshot.riskNotes.map((note) => <li key={note}>{note}</li>)}</ul><strong>{t.final}</strong></aside>
  return <section className={styles.radar} aria-labelledby="market-radar-title">
    ...
    <div className={styles.workspace}>{simpleMode ? <details className={styles.detailed}><summary><span>{t.show}</span><span>{t.hide}</span></summary>{signalPanel}</details> : signalPanel}{riskPanel}</div>
  </section>
}
