...
interface CryptoWatchCandidatesProps {
  ...
  horizonProfile: CandidateHorizonProfile
  onHorizonChange: (horizon: WatchCandidateHorizon) => void
  loading?: boolean
  error?: string | null
...
const text = {
  en: {
    title: 'Crypto Interest Candidates', eyebrow: 'DAILY REVIEW WORKSPACE', subtitle: 'A deterministic research shortlist from Upbit KRW market evidence.', trust: 'Transparent rule-based screen · no AI model · research record only', live: 'LIVE CATALOG', mock: 'MOCK CATALOG', loading: 'Preparing the evidence-ranked watchlist…', empty: 'No complete crypto market catalog is available.', filteredEmpty: 'No candidates match this review filter.', error: 'The crypto catalog could not be loaded.', openMarket: 'Open Market', useMock: 'Use mock catalog', retry: 'Retry', open: 'Open in Market', openAndReview: 'Mark reviewed after opening', why: 'Why it is on watch', evidence: 'Evidence breakdown', risk: 'Risk / invalidation', next: 'Next watch points', news: 'News provenance', source: 'Source', scope: 'Scope', headlines: 'headlines', details: 'Inspect evidence', score: 'Watch Score', today: "Today's review", candidates: 'candidates', dataMode: 'Data mode', newsSource: 'News source', realAi: 'Real AI', inactive: 'Not active', all: 'All', reviewFilter: 'Review status filter', statuses: { unreviewed: 'Unreviewed', watching: 'Watching', reviewed: 'Reviewed', dismissed: 'Dismissed' }, actions: { watching: 'Mark as Watching', reviewed: 'Mark as Reviewed', dismissed: 'Dismiss', unreviewed: 'Reset status' }, note: 'Local note', localOnly: 'Saved on this device only · shared across horizons', addNote: 'Add note', clearNote: 'Clear note', reset: 'Reset local feedback', resetConfirm: 'Clear all candidate statuses and local notes on this device?', checklist: 'Review checklist', checklistItems: ['Check volume confirmation', 'Check BTC direction', 'Check news source', 'Check invalidation condition', 'Open chart before deciding'], analysisHorizon: 'Analysis horizon', cadenceGuidance: 'Candidate refresh guidance', realtimeGuidance: 'Market data can change. Use the snapshot record for comparison.', lifecycle: 'Lifecycle status', lifecycleLabels: { new: 'New', maintained: 'Maintained', strengthened: 'Strengthened', weakened: 'Weakened', 'review-needed': 'Change check needed' }, planning: 'Planning reference', interest: 'Observation context', secondInterest: 'Additional context', target: 'Follow-up context', invalidation: 'Risk review context', planningOnly: 'Rule-based research reference only. Not a trade instruction.', riskReward: 'Risk context', confidence: 'Evidence note', lockedAi: ['AI Analyze · Planned', 'AI News Summary · Planned', 'Deep Dive · Planned'], lockedHelp: 'Real AI is not connected. Future paid-plan credits may be required.' },
  ko: {
    title: '가상자산 관심 후보 목록', eyebrow: '일일 검토 작업공간', subtitle: '업비트 KRW 시장 근거를 규칙 기반으로 계산한 리서치 후보 목록입니다.', trust: '투명한 규칙 기반 화면 · AI 모델 없음 · 리서치 기록 전용', live: '실시간 카탈로그', mock: '모의 카탈로그', loading: '근거 기반 관찰 목록을 준비하고 있습니다…', empty: '사용 가능한 가상자산 시장 카탈로그가 없습니다.', filteredEmpty: '이 검토 필터에 맞는 후보가 없습니다.', error: '가상자산 카탈로그를 불러오지 못했습니다.', openMarket: '마켓 열기', useMock: '모의 카탈로그 사용', retry: '다시 시도', open: '마켓에서 열기', openAndReview: '연 후 검토 완료', why: '관찰 이유', evidence: '근거 점수 상세', risk: '위험 / 무효화 조건', next: '다음 확인 항목', news: '뉴스 출처', source: '출처', scope: '범위', headlines: '개 헤드라인', details: '근거 펼쳐보기', score: '관찰 점수', today: '오늘의 검토', candidates: '개 후보', dataMode: '데이터 모드', newsSource: '뉴스 출처', realAi: '실제 AI', inactive: '비활성', all: '전체', reviewFilter: '검토 상태 필터', statuses: { unreviewed: '미확인', watching: '관찰 중', reviewed: '검토 완료', dismissed: '제외' }, actions: { watching: '관찰 중으로 표시', reviewed: '검토 완료로 표시', dismissed: '제외', unreviewed: '상태 초기화' }, note: '로컬 메모', localOnly: '이 기기에만 저장됩니다 · 분석 기간 간 공유', addNote: '메모 추가', clearNote: '메모 삭제', reset: '로컬 피드백 초기화', resetConfirm: '이 기기의 모든 후보 상태와 로컬 메모를 삭제할까요?', checklist: '검토 체크리스트', checklistItems: ['거래량 확인', 'BTC 방향 확인', '뉴스 출처 확인', '무효화 조건 확인', '판단 전 차트 열기'], analysisHorizon: '분석 기간', cadenceGuidance: '후보 검토 주기', realtimeGuidance: '시장 데이터는 바뀔 수 있습니다. 기준 기록과 비교해 검토하세요.', lifecycle: '수명주기 상태', lifecycleLabels: { new: '신규', maintained: '유지', strengthened: '강화', weakened: '약화', 'review-needed': '변화 확인 필요' }, planning: '계획 참고', interest: '관찰 맥락', secondInterest: '추가 맥락', target: '후속 확인 맥락', invalidation: '위험 검토 맥락', planningOnly: '규칙 기반 리서치 참고용이며 거래 지시가 아닙니다.', riskReward: '위험 맥락', confidence: '근거 참고', lockedAi: ['AI 분석 · 예정', 'AI 뉴스 요약 · 예정', '심층 분석 · 예정'], lockedHelp: '실제 AI는 아직 연결되지 않았습니다. 향후 유료 플랜의 크레딧이 필요할 수 있습니다.' },
} as const
...
export function CryptoWatchCandidates({ candidates, language, mode, newsSource, horizon, horizonProfile, onHorizonChange, loading = false, error = null, onOpenInstrument, onOpenMarket, onModeChange, onRetry, newsInsights = [] }: CryptoWatchCandidatesProps) {
  ...
  return <main className={styles.page}>
    ...
    {!loading && !error && visible.length > 0 && <ol className={styles.list} aria-label={t.title}>{visible.map((candidate) => { const item = feedback[candidate.instrumentId]; const status = item?.status ?? 'unreviewed'; return <li key={candidate.id} className={styles.card} data-review-status={status}><div className={styles.body}>
      <header className={styles.cardHeader}><div><strong>{candidate.symbol}</strong><span>{candidate.name}</span><em data-review-status={status}>{t.statuses[status]}</em><em data-lifecycle={candidate.lifecycleStatus}>{t.lifecycleLabels[candidate.lifecycleStatus]}</em></div></header>
      <p className={styles.reason}><b>{t.why}</b>{candidate.watchReason}</p><div className={styles.chips}>{candidate.evidence.slice(0, 4).map((evidence) => <span key={evidence.type} data-status={evidence.status}>{evidence.label}</span>)}</div><div className={styles.snapshot}><p><b>{t.risk}</b>{candidate.riskSummary}</p><p><b>{t.news}</b>{candidate.newsEvidence.source} · {candidate.newsEvidence.scope}</p></div>
