...
const copy = {
  en: { title: 'Stock Watch Candidates', korea: 'Korea Stock Candidates', us: 'US Stock Candidates', eyebrow: 'EARLY BETA · RULE-BASED', coverage: 'Stock candidate coverage is in early beta. Some data may be mock, limited, or unavailable. Use this screen to review the workflow, not as live investment guidance.', empty: 'Stock watch candidates are not available yet. Stock data coverage is still being expanded.', loading: 'Preparing limited stock catalog evidence…', score: 'Watch Score', quality: 'Data quality', sector: 'Sector / theme', unavailable: 'Not available', why: 'Why watch', evidence: 'Evidence breakdown', risk: 'Risk summary', news: 'News evidence', planning: 'Planning reference only', interest: 'Interest area', target: 'Target observation area', invalidation: 'Invalidation / risk area', cadence: 'Review cadence', open: 'Open in Market', advice: 'Not investment advice', status: { unreviewed: 'Unreviewed', watching: 'Watching', reviewed: 'Reviewed', dismissed: 'Dismissed' }, actions: { watching: 'Mark as Watching', reviewed: 'Mark as Reviewed', dismissed: 'Dismiss' }, note: 'Local note', noteHelp: 'Saved on this device only', qualityLabels: { live: 'Live data', mock: 'Mock / preview data', limited: 'Limited data', unavailable: 'Unavailable' } },
  ko: { title: '주식 관찰 후보', korea: '한국 주식 관찰 후보', us: '미국 주식 관찰 후보', eyebrow: '초기 베타 · 규칙 기반', coverage: '주식 후보 기능은 초기 베타입니다. 일부 데이터는 모의·제한·미지원 상태일 수 있습니다. 이 화면은 실시간 투자 판단이 아니라 분석 흐름 검토용입니다.', empty: '주식 관찰 후보는 아직 준비 중입니다. 주식 데이터 범위를 확장하는 중입니다.', loading: '제한된 주식 카탈로그 근거를 준비하고 있습니다…', score: '관찰 점수', quality: '데이터 품질', sector: '섹터 / 테마', unavailable: '정보 없음', why: '관찰 이유', evidence: '근거 상세', risk: '위험 요약', news: '뉴스 근거', planning: '계획 참고용', interest: '관심 구간', target: '목표 관찰 구간', invalidation: '무효화 / 위험 구간', cadence: '검토 주기', open: '마켓에서 열기', advice: '투자 조언 아님', status: { unreviewed: '미확인', watching: '관찰 중', reviewed: '검토 완료', dismissed: '제외' }, actions: { watching: '관찰 중으로 표시', reviewed: '검토 완료로 표시', dismissed: '제외' }, note: '로컬 메모', noteHelp: '이 기기에만 저장됩니다', qualityLabels: { live: '실시간 데이터', mock: '모의 / 미리보기 데이터', limited: '제한된 데이터', unavailable: '미지원' } },
} as const
...
export function StockWatchCandidates({ candidates, region, language, horizon, supportedInstrumentIds, loading = false, onOpenInstrument }: StockWatchCandidatesProps) {
  ...
  return <section className={styles.page} aria-labelledby="stock-candidates-title">
    ...
    {!loading && candidates.length > 0 && <ol className={styles.list}>{candidates.map((candidate) => { const item = feedback[candidate.instrumentId]; const status = item?.status ?? 'unreviewed'; const supported = supportedInstrumentIds.has(candidate.instrumentId); return <li key={candidate.id} className={styles.card} data-status={status}>
      <div className={styles.rank}>{String(candidate.rank).padStart(2, '0')}</div><div className={styles.body}><header><div><strong>{candidate.symbol}</strong><span>{candidate.name}</span><em>{region === 'korea' ? 'KOSPI / KOSDAQ' : 'NASDAQ / NYSE'}</em></div><div className={styles.score}><span>{t.score}</span><b>{candidate.watchScore}</b></div></header>
      ...
      <details><summary>{t.evidence}</summary><dl>{candidate.evidence.map((evidence) => <div key={evidence.type}><dt>{evidence.label}</dt><dd><b>{evidence.score}/{evidence.maxScore}</b>{evidence.summary}</dd></div>)}</dl><h2>{t.news}</h2><p>{candidate.newsEvidence.scope} · {candidate.newsEvidence.count}</p><h2>{t.risk}</h2><p>{candidate.riskSummary}</p></details>
      <section className={styles.planning}><header><b>{t.planning}</b><span>{t.cadence}: {candidate.reviewCadence}</span></header><div><p><b>{t.interest}</b>{candidate.planningZones.interestArea}</p><p><b>{t.target}</b>{candidate.planningZones.targetObservationArea}</p><p><b>{t.invalidation}</b>{candidate.planningZones.invalidationRiskArea}</p></div><small>{candidate.planningZones.riskRewardNote}</small></section>
