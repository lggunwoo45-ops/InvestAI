...
const text = {
  en: {
    ...
    verify: 'Verify primary-source market and company information before deciding.', revisit: 'Revisit this analysis when new evidence becomes available.', risk: 'Check downside risk and your own decision criteria.', largeMove: 'Large recent move — review volatility before making any decision.', limited: 'Data quality is limited.', dataQuality: 'Data quality', userNotice: 'User-provided context only. Not market evidence.', intentNotice: 'Review intent changes checklist wording only. It does not create personal investment advice.', disclaimer: 'Decision-support structure only. No real AI model is connected. Not investment advice; the user makes the final decision.',
    intents: { watching: 'Watching', holding: 'Holding', longTerm: 'Long-term review', swing: 'Swing review', shortTerm: 'Short-term review' },
    intentChecks: { watching: 'Check whether new evidence keeps this asset relevant.', holding: 'Review whether your original reason still matches available evidence.', longTerm: 'Revisit when filings, earnings, or fundamentals are connected.', swing: 'Recheck movement and volume after large changes.', shortTerm: 'Recheck recent movement and news context before deciding.' },
  },
  ko: {
    ...
    verify: '판단 전에 시장·기업의 1차 출처를 직접 확인하세요.', revisit: '새 근거가 생기면 이 분석을 다시 검토하세요.', risk: '하방 위험과 본인의 판단 기준을 확인하세요.', largeMove: '최근 움직임이 큽니다. 판단 전 변동성을 확인하세요.', limited: '데이터 품질이 제한적입니다.', dataQuality: '데이터 품질', userNotice: '사용자 입력 참고값입니다. 시장 근거가 아닙니다.', intentNotice: '검토 목적은 체크리스트 문구만 바꾸며 개인 투자 조언을 생성하지 않습니다.', disclaimer: '의사결정 보조 구조일 뿐입니다. 실제 AI 모델은 연결되지 않았으며 투자 조언이 아닙니다. 최종 판단은 사용자가 합니다.',
    intents: { watching: '관찰 중', holding: '보유 중', longTerm: '장기 검토', swing: '스윙 검토', shortTerm: '단기 검토' },
    intentChecks: { watching: '새로운 근거에서도 이 종목을 계속 살펴볼 이유가 유지되는지 확인하세요.', holding: '처음 검토한 이유가 현재 확인 가능한 근거와 여전히 맞는지 확인하세요.', longTerm: '공시·실적·재무 데이터가 연결되면 다시 검토하세요.', swing: '큰 움직임 뒤 가격 변화와 거래량을 다시 확인하세요.', shortTerm: '판단 전에 최근 움직임과 뉴스 맥락을 다시 확인하세요.' },
  },
} as const
...
export function buildMyInstrumentAnalysis(input: MyAnalysisEngineInput): MyAnalysisResult {
  ...
  const dataCaution: string | null = mock ? t.mockValue : dataQuality === 'limited' || dataQuality === 'unavailable' ? t.limited : null
  const risks: string[] = []
  if (dataCaution) risks.push(dataCaution)
  if (Math.abs(instrument.change24hPercent) >= 8) risks.push(t.largeMove)
  risks.push(t.risk)
